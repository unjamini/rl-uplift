...
from metrics import qini_discrete
...
class RLift:
    def __init__(self,
        ...
        self.log = open('log_' + time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime()), 'a')
        ...
        for epoch in range(self.max_epoch):
            ...
            for eid in range(self.n_step_repeat):
                ...
                for a, data, p in zip(actions, bag, probs):
                    ...
                    records.append([int(a), data[1], {'reward': data[2]}, real_probs, p])
            ...
            print("Getting qini for test")
            qini_score = self.test(
                eval_func=self.test_eval_func,
                datas=self.tests,
                epoch=epoch,
                result_output=False,
                return_qini=True,
            )
            print(f"Epoch: {epoch} qini score {qini_score}")
            self.qini_scores.append(qini_score)
    ...
    def test(self, datas, epoch, eval_func, output_filename=None, result_output=False, return_qini=False):
        '''
        ...
        if return_qini:
            return qini_discrete(record=records)
